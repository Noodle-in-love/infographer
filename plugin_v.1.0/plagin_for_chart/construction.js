...
function init() {
  ...
  //Вычисляем и сохраняем начальный и конечный угол каждого сектора в диаграмме
  ...
  //Выводим диаграмму и добавляем обработчик события click к ней
  drawChart();
...
function rectChart() {
	//конфигурация
	  // Конфигурация
  var chartSizePercent = 55;                        // Длинна диаграммы, выраженный в процентах от размеров области рисования
  var sliceBorderWidth = 1;                         // Ширина (в пискселях) границы вокруг каждого сектора
  var sliceBorderStyle = "#fff";                    // Цвет границы вокруг каждого сектора
  var maxPullOutDistance = 25;                      // Насколько далеко будет выдвигаться сектор из диаграммы
  var pullOutFrameStep = 4;                         // На сколько пикселей перемещается сектор в каждом кадре анимации
...
function init() {
  ...
  if ( typeof canvas.getContext === 'undefined' ) return;
  // Инициализуем некоторые свойства области рисования и диаграммы
  canvasWidth = canvas.width;   // Задаём ширину области рисования
  canvasHeight = canvas.height; // Задаём высоту области рисования
  /*
   * Получаем данные из таблицы
   * и устанавливаем обработчики события click для ячеек таблицы
   */
  ...
  $('#chartData td').each( function() {
    ...
    if ( rgb = $(this).css('color').match( /rgb\((\d+), (\d+), (\d+)/) ) {
      ...
      return;
                      ...
                      //Вычисляем и сохраняем начальную  и конечную точку в каждом из столбцов
  var currentPos = 0; //Текущая позиция сектора (от 0 до 1)
  for ( var slice in chartData ) {
    chartData[slice]['startAngle'] = 2 * Math.PI * currentPos;
    chartData[slice]['endAngle'] = 2 * Math.PI * ( currentPos + ( chartData[slice]['value'] / totalValue ) );
